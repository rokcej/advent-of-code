...
fn main() {
    ...
    let mut network_sizes: Vec<usize> = vec![1; nodes.len()];
    ...
    for pair in &pairs {
        ...
        for node in &mut nodes {
            if node.network_id == old_network_id {
                ...
                network_sizes[old_network_id] -= 1;
                network_sizes[new_network_id] += 1;
            }
        }

        if network_sizes[new_network_id] == nodes.len() {
            result = Some(nodes[pair.i].position.0 * nodes[pair.j].position.0);
            break;
        }
    }
    let result = result.expect("No result found");
    println!("{result}");
}
